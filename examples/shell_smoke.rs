...
fn smoke(bin: &Path, out_dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    // above the tools (ADR-0010) and its `defaults.default_role: "coder"`
    // preselects index 1 (ADR-0012; rows are alphabetical: advisor, coder,
    // researcher — this harness assumes no `.swarm/swarm.local.json` overlay
    // shifts the plan). Selecting a role would inject startup commands into
    // a real CLI, so navigate past them: 2×`j` from "coder" lands on the
    ...
    // 8. Second tool: picker (preselected on "coder" again) → 3×down (past
    // the remaining role and Claude Code) → Enter selects the agy tool row,
    // whose options form (model only) opens; Enter with an empty model
    // launches defaults. While the repo dir is untrusted, agy boots to its
    // workspace-trust dialog; after the owner trusts it (milestone 2b stage
    // 2), the main UI boots instead — both paint a Gemini model name, so
    // wait on that and report which state.
    d.send(PREFIX)?;
    d.send(b"c")?;
